...
import demoRoute from './map/demoRoute'
import bussRoute from './map/bussRoute'

var staticRoute = [].concat(commRoute, bussRoute, demoRoute)
...
function initRoute(router){
    return new Promise((resolve) => {
        if(permissionList.length == 0){
            console.log("没有菜单数据，正在获取中。。。。")
            store.dispatch('auth/getNavList').then(() => {
                console.log("菜单数据获取完成",store.state.auth.navList)
                store.dispatch('auth/getPermissionList').then((res) => {
                    console.log("已经获取菜单权限数据",res)
                    permissionList = res
                    res.forEach(function(v){
                        try {
                            ...
                            console.log("v.error.path =  " + v.path)
                        }
                    })
...
//页面切换时,显示有一条蓝色或者是红色的loading加载进度条
...
router.beforeEach((to, from, next) => {
    ...
    if (Auth.isLogin()) {
        ...
        if (routerWhiteList.indexOf(to.path) >= 0) {
